...
function desactivarModoNina() {
    ...
    // Reactivar TODAS las zonas — forzar repaint para asegurar que el navegador registre el cambio
    document.querySelectorAll('.zona').forEach(z => {
        z.style.pointerEvents = 'auto';
    });
    // Un frame después retirar el override inline para devolver el control al CSS,
    // pero SOLO si el CSS ya contiene una regla que lo cubra; si no, dejamos 'auto'.
    requestAnimationFrame(() => {
        document.querySelectorAll('.zona').forEach(z => {
            // Comprobar si el estilo computado sigue siendo 'auto' antes de limpiar
            const computed = window.getComputedStyle(z).pointerEvents;
            // Limpiar el inline solo si no hay ningún otro modo activo que lo necesite
            if (!ninaModoActivo) {
                z.style.pointerEvents = '';
                // Si el CSS devuelve 'none' (zona sin estilo propio), forzar 'auto'
                if (window.getComputedStyle(z).pointerEvents === 'none') {
                    z.style.pointerEvents = 'auto';
                }
            }
        });
...
(function() {
    ...
    function intentarPantallaCompleta() {
        ...
        // screen.orientation.lock (Chrome Android, requiere fullscreen previo)
    ...
    function verificarOrientacion() {
        ...
        if (esPortrait()) {
            ...
            intentarPantallaCompleta();
            // Activar audio si venía de portrait y aún no había iniciado
            if (ambienteArrancoDesdeOri) {
                reanudarAmbiente();
            }
        }
