...
Object.keys(zonaData).forEach(id => {
    ...
    // Asegurar que el cursor indique elemento clickeable
    el.style.cursor = 'pointer';

    // Click para escritorio
    el.addEventListener('click', () => abrirContenido(zonaData[id]));

    // Touch para móvil: usar touchend con detección de tap (sin desplazamiento)
    ...
    el.addEventListener('touchend', e => {
        ...
        // Solo activar si fue un tap (sin scroll)
        if (dx < 10 && dy < 10) {
            e.preventDefault();
            abrirContenido(zonaData[id]);
        }
    }, { passive: false });
});
