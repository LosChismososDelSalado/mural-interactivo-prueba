...
// ==========================================
// --- GPU ACCELERATION — evita parpadeo en PNG animados ---
// ==========================================
...
// ==========================================
// --- CARGA INICIAL DEL MURAL ---
// Desactiva zonas, gira balón y anima grieta hasta que
// las imágenes críticas estén listas
// ==========================================
(function inicioCargaMural() {
    const zonas = document.querySelectorAll('.zona');

    // 1. Deshabilitar zonas durante carga
    zonas.forEach(z => { z.style.pointerEvents = 'none'; z.style.opacity = '0'; });

    // 2. Imágenes críticas que deben cargar primero
    const imgsCriticas = [
        'assets/mural-fondo-base.webp',
        'assets/p1971-jugadoras.png',
        'assets/f2026-futbolista.png',
        'assets/f2026-repartidora.png',
        'assets/nina-base.png',
    ];

    let cargadas = 0;
    const total  = imgsCriticas.length;
    const durEstimada = 4; // segundos estimados

    // Iniciar animación de carga
    balon.classList.add('girando');
    grietaVerde.style.setProperty('--duracion-carga', durEstimada + 's');
    ...
    function onImagenLista() {
        cargadas++;
        if (cargadas >= total) activarMural();
    }

    function activarMural() {
        if (_muralListo) return; // evitar doble llamada (img.complete + onload)
        _muralListo = true;
        ...
        // Activar zonas con fade in
        zonas.forEach((z, i) => {
            setTimeout(() => {
                z.style.transition   = 'opacity 0.4s ease';
                z.style.pointerEvents = '';
                z.style.opacity      = '';
                setTimeout(() => { z.style.transition = ''; }, 500);
            }, i * 30);
        });
    }

    // Pre-cargar imágenes críticas
    imgsCriticas.forEach(src => {
        const img = new Image();
        let contada = false;
        function contar() { if (!contada) { contada = true; onImagenLista(); } }
        img.onload  = contar;
        img.onerror = contar;
        img.src = src;
        // Si ya está cacheada y complete, contar directamente
        if (img.complete) contar();
    });

    // Fallback: si tarda más de 8s activar de todas formas
    setTimeout(activarMural, 8000);
})();
...
// --- BALÓN Y GRIETA (para carga de contenido individual) ---
let _muralListo = false; // se activa cuando la carga inicial del mural termina
function iniciarCarga(duracionEstimada = 3) {
    if (!_muralListo) return; // durante la carga inicial no interferir
    balon.classList.add('girando');
...
function limpiarEfectoNina() {
    ...
    if (vfx) {
        vfx.innerHTML = '';
        // Garantizar que nina-vfx nunca bloquea eventos al mural
...
function desactivarModoNina() {
    ...
    // Reactivar TODAS las zonas con 'auto' explícito.
    // IMPORTANTE: .zona en el CSS NO declara pointer-events, por lo que
    // limpiar el inline style con '' haría que el browser herede del padre,
    // pudiendo resultar en 'none'. Siempre se deja 'auto' como valor fijo.
    document.querySelectorAll('.zona').forEach(z => {
...
(function() {
    ...
    function crearBloqueador() {
        ...
        // Al tocar el bloqueador: intentar pantalla completa + activar audio ambiente + bloquear scroll
    ...
    function intentarPantallaCompleta() {
        // Scroll a 0 para empujar la barra del navegador hacia fuera de pantalla
        window.scrollTo(0, 1);
        setTimeout(function() { window.scrollTo(0, 0); }, 50);

        // API de pantalla completa (soportada en muchos navegadores móviles)
        ...
        if (fn) {
            fn.call(el).then(function() {
                // Solo intentar lock DESPUÉS de que fullscreen se concedió
                // Esto evita que el navegador active la rotación forzada del sistema
        ...
        // NO llamar screen.orientation.lock sin fullscreen previo
    }
    ...
    function verificarOrientacion() {
        ...
        if (esPortrait()) {
            ...
            quitarBloqueador();
            // Al girar a landscape: activar audio si venía de portrait
            if (ambienteArrancoDesdeOri) {
                reanudarAmbiente();
            }
            // NO llamar intentarPantallaCompleta aquí — eso activa la rotación
            // del sistema sin un gesto del usuario. Solo se llama al tocar el bloqueador.
        }
    }
    ...
    window.addEventListener('resize', function() {
        // Solo reaccionar en móvil para no interferir con resize de escritorio
